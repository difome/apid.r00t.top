...
const API_URL = config.providers.metalchartsApiUrl;
const TIMEOUT = 20000;
...
export class CommodityParserService {
    private isRunning = false;

    private async getAuthViaPlaywright() {
        ...
        try {
            await page.goto("https://metalcharts.org/", { waitUntil: "domcontentloaded", timeout: TIMEOUT });
            
            // Wait for the token to be captured by the response interceptor
            let waitTime = 0;
            while (!token && waitTime < 10000) {
    ...
    private async ensureAuth() {
        ...
        if (!auth || !auth.token) {
            auth = await this.getAuthViaPlaywright();
            if (auth.token) this.saveAuth(auth);
        }
        return auth;
    }

    private async fetchPrices(auth: any, url: string) {
    ...
    private async processData(auth: any, url: string, category: string): Promise<{ success: boolean; logLines: string[] }> {
        const logLines: string[] = [];
        try {
            let res: any = await this.fetchPrices(auth, url);
            ...
            for (const info of items) {
                ...
                if (!price) {
                    logLines.push(`⚠️ ${sym}: ${category} data not available`);
                    continue;
                }
                ...
                if (c) {
                    ...
                    if (lastRate) {
                        if (price > Number(lastRate.price)) direction = 'up';
                        else if (price < Number(lastRate.price)) direction = 'down';
                    } else {
                        direction = 'up';
                    }
                    ...
                    const icon = direction === 'up' ? '✅' : direction === 'down' ? '🔴' : 'ℹ️';
                    logLines.push(`${icon} ${sym}: ${price} (${direction})`);
                }
            }
            return { success: true, logLines };
        } catch (e) {
            console.error(`Failed to process ${category}`, e);
            logLines.push(`Failed to process ${category}: ${e}`);
            return { success: false, logLines };
        }
    }
    ...
    async run() {
        ...
        try {
            ...
            for (const target of targets) {
                let result = await this.processData(auth, target.url, target.category);
                if (!result.success) {
                    console.log("Retrying auth...");
                    auth = await this.getAuthViaPlaywright();
                    this.saveAuth(auth);
                    result = await this.processData(auth, target.url, target.category);
                }
            ...
            status = 'error';
            allLogs.push(`Critical error: ${e.message}`);
        } finally {
            this.isRunning = false;
        }

        const summary = `Commodities sync completed. Processed: ${allLogs.length} items`;
        const message = `${summary}; ${allLogs.join('; ')}`;
        ...
        await prisma.parsingLog.deleteMany({
            where: {
                source: 'commodities',
